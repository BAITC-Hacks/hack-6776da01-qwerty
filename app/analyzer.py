...
def make_summary(transcript: str, tasks: list[dict[str, str]]) -> str:
    ...
    decisions = [item["task"] for item in tasks[:5]]
    task_lines = [f"- {item['task']} — {item['responsible']} ({item['deadline']})" for item in tasks[:5]]
    return "\n".join([
        "Основные сведения",
        f"- Фрагментов речи: {len(lines)}",
        f"- Участники: {', '.join(speakers[:8]) or 'не определены'}",
        f"- Найдено поручений: {len(tasks)}",
        "",
        "Следующие шаги",
        *(task_lines or ["- Поручения не найдены; требуется ручная проверка транскрипта."]),
        "",
        "Проверка перед отправкой",
        "- Подтвердите имена говорящих, ответственных и сроки в таблице поручений.",
    ])
